...
# ======================
# 기본 설정 & 로고 (상단 잘림 방지/중앙 정렬)
# ======================
APP_DIR = Path(__file__).parent
LOGO_FILE = "ts-logo.png"   # 로고 파일명 (app.py 또는 assets/ 폴더)
LOGO_HEIGHT = 48            # 로고 세로 높이(px) — 40~60 선호

# 페이지 설정(가장 먼저)
logo_path = None
for cand in [APP_DIR / LOGO_FILE, APP_DIR / "assets" / LOGO_FILE]:
    if cand.exists():
        logo_path = str(cand); break
...
def _img_b64(p):  # 로고를 base64로 넣어 배포 환경 경로 이슈 줄이기
    with open(p, "rb") as f:
...
def build_summary_two_col(
    ...
    total_subsidy, avg_subsidy_per_unit, total_gross_capex,
    is_tiered,
    ...
    m_total_inv_on, m_total_inv_val,
    m_total_sub_on, m_total_sub_val,
    grant_surplus, subsidy_coverage_ratio, gross_payback_months
) -> pd.DataFrame:
    """항목/값 2열 요약 테이블 (요약 시트에 열로 붙이는 원본)"""
...
if submitted:
    ...
    grant_surplus = max(total_subsidy - total_gross_capex_auto, 0.0)  # 초기 순현금 유입
    initial_investment = max(initial_investment, 0.0)                  # 음수면 0으로
    ...
    st.success("완료! 다운로드 버튼으로 결과 파일을 저장하세요. (마스터 엑셀을 올렸다면 그 위에 이어붙여서 내려갑니다.)")
