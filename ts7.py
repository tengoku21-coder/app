# -*- coding: utf-8 -*-
"""
EV Charger Business Analyzer (Web / Streamlit)
- 총 사용량 단위 선택(정산기간 총량/월 총량) + 사용률 경고
- 프로모션 기간/단가 + 장기(정상요금) 지표 동시 계산
- 회수기간: 월별 시뮬레이션(프로모션 → 정상요금)으로 동적 계산
- 엑셀: '분석 요약(열 누적)' + 'History(행 누적)'
...
if submitted:
    ...
    y1_1.metric("총 월 순이익(가중)", f"{monthly_profit_blended_total:,.0f} 원/월")
    y1_2.metric("1기당 연간 순이익(가중)", f"{profit_1y_per_charger_blended:,.0f} 원/년")
    y1_3.metric("ROI(연간, 가중)", "N/A" if roi_per_charger_blended is None else f"{roi_per_charger_blended:.2f} %")
    ...
    ss_3.metric("ROI(연간, 정상요금)", "N/A" if roi_per_charger_steady is None else f"{roi_per_charger_steady:.2f} %")
    ...
    history_row = {
        ...
        'ROI(연간, 1년차 가중, %)': None if roi_per_charger_blended is None else float(roi_per_charger_blended),
        'ROI(연간, 정상요금, %)': None if roi_per_charger_steady  is None else float(roi_per_charger_steady),
        # 원가/보조금/상수
